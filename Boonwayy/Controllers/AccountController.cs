    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl, FormCollection formCollection)
        {
            ...
            else
            {
                ...
                if (userMail != null && model.Email != "admin@boonwayy.com")
                {
                    if (!await UserManager.IsEmailConfirmedAsync(userMail.Id))
                    {
                ...
                try
                {
                    if (String.IsNullOrEmpty(returnUrl))
                    {
                        ...
                        {
                            return RedirectToAction("PostProject", "Manage");
                        }
                    }
                    else
                    {
                ...
                //        if (UserManager.IsInRole(user.Id, "Admin"))
                //        {
                //            var myRole = UserManager.IsInRole(user.Id, "Admin");
                //            if (myRole)
                //            {
                //                ViewBag.Role = "Admin";
                //            }
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    var userProfile = new UserProfile {
                        FullName = model.FullName, ApplicationUserId = user.Id, Gender = "N/A", Age = 0, SocialSecurityNumber = "N/A", Profession = "N/A", ProfilePictureUrl = "default-user.png"
                    };
                    ...
                    catch (DbEntityValidationException DbEx)
                    {
                        foreach (var validationErrors in DbEx.EntityValidationErrors)
                        {
                            foreach (var validationError in validationErrors.ValidationErrors)
                            {
                                ...
                                Response.Write(validationError.PropertyName);
                    ...
                    message = message.Replace("@ViewBag.Name", CultureInfo.CurrentCulture.TextInfo.ToTitleCase(model.FullName));
                    message = message.Replace("@ViewBag.Link", callbackUrl);

                    //var urlPolicyPage = Url.Action("Privacy", "Home", Request.Url.Scheme);
                    //var urlTermsPage = Url.Action("Terms", "Home", Request.Url.Scheme);

                   

                    //message = message.Replace("@ViewBag.Policy", urlPolicyPage);
                    //message = message.Replace("@ViewBag.Terms", urlTermsPage);

                    await UserManager.SendEmailAsync(user.Id, "Confirm your account", message);

                    TempData["MailConfirm"] = "A confirmation mail has been sent to " + user.Email + " \n please confirm your account.";
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                var db = new ApplicationDbContext();
                var userProfile = db.UserProfiles.Where(aid => aid.ApplicationUserId == user.Id).FirstOrDefault();

                var message = await EmailTemplate("ForgotPassword");
                message = message.Replace("@ViewBag.Name", CultureInfo.CurrentCulture.TextInfo.ToTitleCase(userProfile.FullName));
                message = message.Replace("@ViewBag.Link", callbackUrl);
